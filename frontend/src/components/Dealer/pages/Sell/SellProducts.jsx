...
function SellProducts() {
    ...
    const Product_Stat = (Product) => {
        return (
            <>
                {
                    Product.is_dealer_sold === 1 ? <Badge severity={"success"} value={"Sold"}  /> : <Badge severity={'danger'} value={"Not Sold"} />
                }
            </>
        )
    }
        ...
        <div>
            <Card title="Product Sell">
                <DataTable loading={loading} value={Product} rows={8} paginator paginatorLeft>
                    ...
                    <Column field='is_dealer_sold' body={Product_Stat} header="Product Status"></Column>
